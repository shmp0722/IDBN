...
setenv('SUBJECTS_DIR', '/media/USB_HDD1/HCP/mgh_1002/T1w'); % be sure to edit this to your path


%% cd to subject directory and generate a list of subject names

Subs = dir('mgh_1002'); % change this to a base name that will pull all participants
    ...
    for subnum=1:length(Subs) % for all participants
        if ischar(Subs);
             eval(['! mri_label2label --srcsubject fsaverage --srclabel fsaverage/label/' labelslh{labelnum}...
                ' --trgsubject ' Subs ' --trglabel ' Subs '/label/' labelslh{labelnum} ...
    ...
    for subnum=1:length(Subs) % for all participants
        if ischar(Subs)
             eval(['! mri_label2label --srcsubject fsaverage --srclabel fsaverage/label/' labelsrh{labelnum}...
                ' --trgsubject ' Subs ' --trglabel ' Subs '/label/' labelsrh{labelnum} ...
...
for labelnum=1:length(labelslh) % for all labels
    for subnum=1:length(Subs) % for all participants
        if ischar(Subs)
            eval(['! mris_anatomical_stats -l ' labelslh{labelnum} ' -f ' statsoutfolder '/' Subs '.' labelslh{labelnum} '.out ' Subs ' lh'])
...
for labelnum=1:length(labelsrh) % for all labels
    for subnum=1:length(Subs)% for all participants
      if ischar(Subs)
          eval(['! mris_anatomical_stats -l ' labelsrh{labelnum} ' -f ' statsoutfolder '/' Subs '.' labelsrh{labelnum} '.out ' Subs ' rh'])
