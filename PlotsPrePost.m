...
for jj = 29:30% length(afq.fgnames)
    ...
    for ii = 1:4
        ...
        plot(afq.vals.fa{jj}(ii,:),'--b', 'LineWidth',2)%,'Color',c(ii,:))
        plot(afq.vals.fa{jj}(ii+4,:),'-r', 'LineWidth',2)%,'Color',c(ii,:))
...
%         title(sprintf('%s; %s', f(1:k(1)-1), afq.fgnames{jj}(1:3)))
        ...
        saveas(gca, fullfile(pwd,'ForSinkeiGanka',sprintf('%s_%s.png',f(1:k(1)-1),afq.fgnames{jj}(1:3) )))
...
for jj = 31:32% length(afq.fgnames)
...
for ii = 1:4 %subject loop
    ...
    % set fg names
    set(gca,'XTickLabelRotation',90,'XTickLabelMode','manual','XTick',...
        [1:(length(afq.fgnames)-4)],...
        'XTickLabel',afq.fgnames(1: (length(afq.fgnames)-4)) )
